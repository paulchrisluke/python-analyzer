import { AppSidebar } from '@/components/app-sidebar'
import { SiteHeader } from '@/components/site-header'
import { SidebarInset, SidebarProvider } from '@/components/ui/sidebar'
import { loadAdminData } from '@/lib/admin-data-server'
import { requireAdmin } from '@/lib/auth-server'
import { unstable_noStore } from 'next/cache'

unstable_noStore()

// Disable caching for sensitive admin data
export const dynamic = 'force-dynamic'
export const revalidate = 0

export default async function MetricsPage() {
  // Enforce admin authentication on the server before loading sensitive data
  const user = await requireAdmin()
  
  // Only load admin data after confirming user is authenticated as admin
  const adminData = await loadAdminData()


  return (
    <SidebarProvider>
      <AppSidebar />
      <SidebarInset>
        <SiteHeader title="Business Metrics" />

        {/* Main Content matching dashboard structure */}
        <div className="flex flex-1 flex-col">
          <div className="@container/main flex flex-1 flex-col gap-2">
            <div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
              {/* Header */}
              <div className="px-4 lg:px-6">
                <h1 className="text-4xl font-bold text-gray-900 mb-2">Business Metrics</h1>
                <p className="text-lg text-gray-600">Financial performance and business analytics</p>
              </div>

              {/* Financial Performance */}
              <div className="px-4 lg:px-6">
                <h2 className="text-2xl font-semibold text-gray-800 mb-4">Financial Performance</h2>
                <pre className="bg-gray-50 p-4 rounded-lg overflow-auto text-sm">{JSON.stringify(adminData.businessMetrics, null, 2)}</pre>
              </div>
              
              {/* Coverage Analysis */}
              <div className="px-4 lg:px-6">
                <h2 className="text-2xl font-semibold text-gray-800 mb-4">Coverage Analysis</h2>
                <pre className="bg-gray-50 p-4 rounded-lg overflow-auto text-sm">{JSON.stringify(adminData.coverageAnalysis, null, 2)}</pre>
              </div>
            </div>
          </div>
        </div>
      </SidebarInset>
    </SidebarProvider>
  )
}
