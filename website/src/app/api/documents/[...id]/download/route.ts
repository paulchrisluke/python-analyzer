...
// GET /api/documents/[...id]/download - Download document content through authenticated proxy
...
export async function GET(
  ...
  try {
    ...
    if (!session) {
      console.warn(`Unauthorized download request for document ${documentId} from ${request.headers.get('x-forwarded-for') || 'unknown IP'}`);
      return NextResponse.json(
    ...
    if (!rateLimit.allowed) {
      console.warn(`Rate limit exceeded for user ${userId} downloading document ${documentId}`);
      return NextResponse.json(
    ...
    if (!hasAccess) {
      console.warn(`Unauthorized download request for document ${documentId} by user ${session.user?.email} with role ${userRole}`);
      return NextResponse.json(
